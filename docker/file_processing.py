...
def input_args(input_argument: str) -> None:
    """
    ...
    if os.path.isfile(input_argument):
        visualization(path_file=input_argument, model_path=args.model_onnx, mode='video processing',
                      path_folder=input_argument)

    elif os.path.isdir(input_argument):
        visualization(path_folder=input_argument, model_path=args.model_onnx, mode='images processing',
        ...
        try:
            ...
            if input_argument == 0:
                visualization(path_folder=str(input_argument), model_path=args.model_onnx, mode='-',
                              path_file=str(input_argument))
            else:
                print('If you want to use a webcam for prediction, you need to use id: 0.')
        except ValueError:
            print('If you want to use a webcam for prediction, you need to use id: 0.')
...
def visualization(model_path: str, mode: str, path_file: str, path_folder: str) -> None:
    ...
    if mode == 'images processing':
        ...
        for image in images:
            ...
            cv2.imwrite(os.path.join(path_save_image, image), img)
        ...
        path_save_video = 'webka'

    os.makedirs(path_save_video, exist_ok=True)
    width = cap.get(cv2.CAP_PROP_FRAME_WIDTH)
    height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    out = cv2.VideoWriter(os.path.join(path_save_video, 'output_video.avi'), fourcc, 30.0, (int(width), int(height)))
    ...
    while True:
        ...
        out.write(frame)
        if cv2.waitKey(1) == ord('q'):   # don't work
            break
    cap.release()
    out.release()
...
if __name__ == '__main__':
    ...
    input_args(input_argument=args.input)
